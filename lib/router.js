        ...
        return this.routes
                .map( route => {
                    if ( route.hasOwnProperty('roles') && route.roles )
                        route.handlers.splice(0, 0, 'controllers.isAuthenticated');
                });
    }
    ...
    load: (app) => {
        ...
        fs.readdirSync(apiDir)
            ...
            .forEach(api => {
                ...
                let combinedControllers = Object.assign(controllers, middleware);
                let routesConfig = require(path.join(apiDir, api, 'routes.json'));
                let moduleRoutes = new routes(routesConfig, combinedControllers).get();
